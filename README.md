...
- **Agent-first 工作台** — Agent 是顶层身份与运行策略，工作区是其下的私有文件/执行隔离边界；主 Agent 与自定义 Agent 都保持 Agent → Workspace 层级
- **多用户隔离** — Per-user 工作区、Per-user IM 通道、RBAC 权限体系、邀请码注册、审计日志，每个用户拥有独立的执行环境
- **多渠道统一路由** — 飞书、Telegram、QQ、钉钉、微信、Discord、WhatsApp 与 Web 统一接入；同一用户可管理多个 Bot 账号
...
| **Discord**  | Gateway + Bot API            | Markdown               | Bot Token 接入、私聊/群聊路由、配对与多账号隔离                                                             |
| **WhatsApp** | Baileys Web 协议             | 文本与媒体             | QR 码扫码登录、账号级会话持久化、断线重连                                                                   |
| **Web**      | WebSocket 实时通信           | 流式 Markdown          | 图片粘贴/拖拽上传、虚拟滚动、Mermaid 图表渲染、图片 Lightbox                                                |

每个用户可创建多个渠道账号。凭据/扫码会话按用户与账号隔离；渠道账号可设置默认 Agent/工作区，具体群聊或话题再通过工作区绑定、会话绑定覆盖。微信和 WhatsApp 均由界面生成二维码扫码接入，不要求用户手工填写协议 Token。

### Agent-first 工作区模型

产品层级为 `Agent → Workspace → Runtime Session`：

- **Agent** — 保存身份提示词、Claude 原生 preset 开关，以及用户 Skills、用户 MCP 和工具能力策略；Provider 与凭据由系统统一管理
- **Workspace** — 归属于一个 Agent，提供宿主机/容器执行模式、文件目录与消息挂载等隔离边界；创建时必须显式选择 Agent
...
- **系统级 Provider 策略** — Provider、模型与凭据由系统统一管理；运行会话按系统负载均衡策略选择并保持粘性绑定
...
- **Per-user 默认运行上下文** — 主 HappyClaw Agent 始终存在；系统为每个用户维护一个内部默认上下文（admin 可使用宿主机模式，member 使用容器模式），产品导航不把它包装成额外的“主工作区”概念
...
- **脚本任务** — 定时任务支持 Agent 和 Script 两种执行类型；Script 模式只允许管理员在有权限的宿主机工作区直接执行 shell 命令
...
- **竖排会话列表** — 工作区与会话分栏展示，支持创建、重命名、删除与大量会话浏览
...
Agent 在运行时可通过内置 MCP Server 与主进程通信。具体工具会根据工作区类型、渠道和用户权限开放：

| 工具                                                                           | 说明                                                   |
| ------------------------------------------------------------------------------ | ------------------------------------------------------ |
| `send_message` / `send_image` / `send_file`                                    | 运行期间向用户或群组发送文本、图片和文件               |
| `schedule_task`                                                                | 创建定时、周期或一次性任务（cron / interval / once）   |
| `list_tasks` / `update_task`                                                   | 查询或原地更新任务；更新使用 revision 防止覆盖并发修改 |
| `pause_task` / `resume_task` / `cancel_task`                                   | 暂停、恢复或软删除任务定义                             |
| `run_task_now` / `stop_task_run`                                               | 幂等地立即运行任务，或只停止指定的当前运行             |
| `restore_task` / `list_task_runs`                                              | 恢复软删除任务，或查询持久化运行历史                   |
| `register_group`                                                               | 注册新群组（仅 admin 主工作区）                        |
| `discord_get_history` / `discord_get_channel_info` / `discord_get_server_info` | 查询 Discord 历史、频道或服务器信息（相关渠道上下文）  |
| `install_skill` / `uninstall_skill`                                            | 安装或卸载 Skill（仅允许的主工作区）                   |
| `memory_append` / `memory_search` / `memory_get`                               | 追加、搜索或读取工作区记忆                             |

### 定时任务

- **三种调度模式** — Cron 表达式 / 固定间隔 / 一次性执行
- **两种执行类型** — Agent（启动完整 Claude Agent）/ Script（直接执行 shell 命令；仅管理员有权限的宿主机工作区可用，绝不会从容器模式降级到宿主机执行）
...
- **安全立即运行** — Web、REST 与 MCP 共享幂等键和稳定 `runId`，网络重试不会重复创建同一次运行
- **通知渠道** — 任务完成后可通知到指定 IM 渠道（飞书/Telegram/QQ/钉钉/微信）；通知状态与执行状态分离，失败时只重试通知
- **完整的运行历史** — 展示耗时、执行状态、通知状态、尝试次数与结果；支持停止当前运行、软删除和恢复
...
- **项目上下文 MCP** — 工作区可保留项目自带的 MCP 配置，随项目加载；Agent 策略控制 HappyClaw 额外附加的用户 MCP
- **项目上下文 Skills** — 工作区中的 `.claude/skills` 和 `CLAUDE.md` 随项目加载；Agent 策略控制 HappyClaw 额外附加的用户 Skills
- **项目环境变量** — 工作区可配置项目运行所需的普通环境变量；模型、Provider 与凭据由系统统一管理
...
- **HappyClaw 内置 Skills** — 随运行镜像或宿主机工具缓存提供，作为基础能力加载
- **宿主机 Skills** — 管理员选择继承宿主机 Claude 配置时，复用 `~/.claude/skills/`；普通用户不可见、不可挂载
- **用户级附加 Skills** — 每个 HappyClaw 用户拥有独立能力库，可从 skills.sh、HTTPS Git 仓库或 ZIP 导入，并记录来源、版本与安装时间
- **工作区级 Skills** — 项目自带的 `.claude/skills/` 随工作区加载，不在全局能力库中重新分配
- **Agent 能力策略** — 每个 Agent 可继承、精选或关闭用户级附加 Skills；这些能力始终与宿主机/项目配置叠加，最终动作仍受 Agent 工具边界限制
- **最终生效预览** — Agent 配置页可按工作区查看各来源的实际合并结果、同名覆盖、MCP 关闭状态和上下文来源
...
| 能力           | 说明                                                                           |
| -------------- | ------------------------------------------------------------------------------ |
| **用户隔离**   | 每个用户拥有独立的默认运行上下文（内部 `home-{userId}`）、工作目录、IM 通道    |
...
- Discord Bot Token — 仅 Discord 集成需要，前往 Discord Developer Portal 创建
- 微信与 WhatsApp 不需要预先获取 Token，在 HappyClaw 中扫码授权即可
...
3. **配置渠道账号**（可选）— 飞书 / Telegram / QQ / 钉钉 / 微信 / Discord / WhatsApp；每个渠道可添加多个账号
...
4. 发布应用版本并等待审批通过
5. 在 HappyClaw Web 界面的「设置 → 渠道账号」中添加飞书账号并填入 App ID 和 App Secret

每个用户可维护自己的多个飞书 Bot；相同应用凭据不能同时被多个活动账号重复占用。
...
2. 记录返回的 Bot Token
3. 在 HappyClaw Web 界面的「设置 → 渠道账号」中添加 Telegram 账号并填入 Bot Token
...
4. 在 HappyClaw Web 界面的「设置 → 渠道账号」中添加 QQ 账号并填入 App ID 和 App Secret
...
5. 在 HappyClaw Web 界面的「设置 → 渠道账号」中添加钉钉账号并填入 Client ID 和 Client Secret

> 钉钉 Bot 支持单聊和群聊。群聊中需要 @机器人 才会响应。支持 AI Card 流式打字机效果。

### 配置微信集成

1. 在 HappyClaw Web 界面的「设置 → 渠道账号」中选择微信并创建账号
2. 点击「生成二维码」
3. 使用微信扫描二维码并在界面等待授权完成

> 微信消息长度限制为 2000 字符，超出部分自动分片发送。

### IM 斜杠命令

支持命令的渠道中，以 `/` 开头的消息会被拦截为斜杠命令（未知命令继续作为普通消息处理）：
...
| 模式           | 说明                                          | 适用对象                                  | 前置要求                     |
| -------------- | --------------------------------------------- | ----------------------------------------- | ---------------------------- |
| **宿主机模式** | Agent 直接在宿主机运行，访问本地文件系统      | admin 主工作区                            | Claude Agent SDK（自动安装） |
...
**数据流**：消息从 Web 或 7 个 IM 渠道进入主进程，经去重和路由后分发到并发队列。队列通过提供商池选择 API 密钥，启动宿主机进程或 Docker 容器。容器内的 agent-runner 调用 Claude Agent SDK 的 `query()` 函数。流式事件（思考、文本、工具调用等 14 种类型）通过 stdout 标记协议传回主进程，再经 WebSocket 广播到 Web 客户端或通过 IM API 回复到各渠道。MCP Server 通过基于文件的 IPC 通道提供 12 个工具，实现 Agent 与主进程的双向通信。计费引擎在请求前检查配额和余额。
...
│   ├── routes/                   #   16 个路由模块（auth / groups / files / config / monitor /
...
| 项目         | 目录                      | 用途                                |
| ------------ | ------------------------- | ----------------------------------- |
| 主服务       | `/`（根目录）             | 后端服务（16 个路由模块）           |
| Web 前端     | `web/`                    | React SPA（18 个页面、15 个 Store） |
